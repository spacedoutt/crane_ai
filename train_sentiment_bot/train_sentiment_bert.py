...
# Load dataset
df = pd.read_csv('data.csv')#, names=['Sentiment', 'Sentence'], encoding='ISO-8859-1')

# Map sentiment labels to numerical values including 'neutral'
label_mapping = {'positive': 1, 'neutral': 0.5, 'negative': 0}
...
train_df, test_df = train_test_split(df, test_size=0.10, random_state=42)  # Using a very small subset for debugging
...
def preprocess_function(examples):
    return tokenizer(examples['Sentence'].tolist(), truncation=True, padding=True, max_length=32)
...
# Load model
model = AutoModelForSequenceClassification.from_pretrained("distilbert-base-uncased", num_labels=3)  # Adjust num_labels to 3
...
# Save the model
model.save_pretrained("./financial_sentiment_model_bert_new_set")
tokenizer.save_pretrained("./financial_sentiment_model_bert")
